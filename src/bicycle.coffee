...
class T extends G
  constructor: (@data, @depth = 0, options) ->
    ...
    @_headerComponent = @addChild new HR @data.c, htmlWidth: @options.treeWidth - @options.treeDepthShift * @depth
    if @data.d
      @_childrenComponent = @addChild new G groupChildMargin: @options.treeFlatMargin
      @childTrees = (@_childrenComponent.addChild(new T(childData, @depth + 1, options)) for childData in @data.d)
